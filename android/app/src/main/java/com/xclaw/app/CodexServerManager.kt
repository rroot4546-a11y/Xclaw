...
class CodexServerManager(private val context: Context) {
    ...
    fun isClaudeCodeInstalled(): Boolean {
        val paths = BootstrapInstaller.getPaths(context)
        return File(paths.prefixDir, "lib/node_modules/@anthropic-ai/claude-code/package.json").exists()
    }
...
#!/data/user/0/com.xclaw.app/files/usr/bin/sh
exec /data/user/0/com.xclaw.app/files/usr/bin/node /data/user/0/com.xclaw.app/files/usr/lib/node_modules/@openai/codex/bin/codex.js "${'$'}@"
WEOF
...
#!/data/user/0/com.xclaw.app/files/usr/bin/sh
exec /data/user/0/com.xclaw.app/files/usr/bin/node /data/user/0/com.xclaw.app/files/usr/lib/node_modules/npm/bin/npm-cli.js "${'$'}@"
WEOF
...
H3
    ...
    fun installClaudeCode(onProgress: (String) -> Unit): Boolean {
        ...
        val code = runInPrefix(
            "node $npmCli install -g @anthropic-ai/claude-code@latest 2>&1",
            onOutput = { onProgress(it) },
        )
        if (code != 0) {
...
WEOF
    ...
    /**
     * Start the Xclaw Node.js gateway. Serves the Xclaw web UI, exposes the
     * agent + update APIs, and proxies /codex to the codex-web-local server.
     */
    fun startXclawGateway(): Boolean {
        if (gatewayProcess != null) return true

        val paths = BootstrapInstaller.getPaths(context)

        // Extract gateway script and web UI (overwrite every launch).
        try {
            context.assets.open("xclaw.js").use { input ->
                File(paths.homeDir, "xclaw.js").outputStream().use { output ->
                    input.copyTo(output)
                }
            }
            val webDir = File(paths.homeDir, "xclaw-web")
            webDir.deleteRecursively()
            webDir.mkdirs()
            extractAssetDir("xclaw-web", webDir)
        } catch (e: Exception) {
            Log.e(TAG, "Failed to extract Xclaw gateway assets: ${e.message}")
            return false
        }
        ...
        env["HTTPS_PROXY"] = "http://127.0.0.1:$PROXY_PORT"
        env["HTTP_PROXY"] = "http://127.0.0.1:$PROXY_PORT"
        env["XCLAW_APP_VERSION"] = "1.0.0"
        env["CODX_WEB_PORT"] = SERVER_PORT.toString()

        val shell = "${paths.prefixDir}/bin/sh"
        val command = "exec node ${paths.homeDir}/xclaw.js --port $GATEWAY_PORT"

        Log.i(TAG, "Starting Xclaw gateway: $command")
        ...
        Thread {
            ...
            Log.i(TAG, "Gateway process exited with code: ${proc.waitFor()}")
        }.start()

        return true
    }

    fun waitForGateway(timeoutMs: Long = 30_000): Boolean {
        val deadline = System.currentTimeMillis() + timeoutMs
        val url = URL("http://127.0.0.1:$GATEWAY_PORT/api/status")
        ...
        Log.e(TAG, "Gateway did not become ready within ${timeoutMs}ms")
        return false
    }

    fun stopXclawGateway() {
        val proc = gatewayProcess ?: return
        gatewayProcess = null
        try {
            proc.destroy()
        } catch (e: Exception) {
            Log.w(TAG, "Error destroying gateway process: ${e.message}")
        }
        try {
            proc.waitFor()
        } catch (_: InterruptedException) {
